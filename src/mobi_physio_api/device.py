...
class PluxDevice:
    ...
    def _connect_device(self) -> None:
        ...
        if (
            last_error
            and "communication port could not be initialized" in str(last_error).lower()
        ):
            msg = (
    ...
    def _cleanup_processes(self) -> None:
        ...
        try:
            if platform.system() == "Windows":
                ...
                subprocess.run(
                    ["pkill", "-f", "bth_macprocess"], check=False, timeout=5
                )
                # Only kill mobi-physio-api processes, not all uv processes
                subprocess.run(
                    ["pkill", "-f", "mobi-physio-api"], check=False, timeout=5
                )
